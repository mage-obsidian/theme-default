...
test.describe("dashboard", () => {
    ...
    test("greets the customer and lays the metrics out as a spec sheet", async ({ page }) => {
    ...
    test("shows the last order with its thumbnails and status", async ({ page }) => {
    ...
    test("carries contact details, default addresses and the shortcuts", async ({ page }) => {
    ...
    test("every panel heading is an h2, so the outline stays flat under the h1", async ({ page }) => {
